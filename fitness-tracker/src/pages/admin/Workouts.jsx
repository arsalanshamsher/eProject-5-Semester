...
export default function Workouts() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = getToken();
      if (editingWorkout) {
        await axios.put(`http://localhost:5000/api/workouts/${editingWorkout._id}`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
    ...
    <div className="p-6 max-w-7xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              {formData.exercises.map((exercise, index) => (
                <div key={index} className="grid grid-cols-1 md:grid-cols-5 gap-3 p-4 bg-white/10 rounded-lg">
                  <input
                    type="text"
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="p-2 rounded border border-gray-300 focus:ring-2 focus:ring-blue-500 flex-1"
                    />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {workouts.map((workout) => (
          <div key={workout._id} className="bg-white/20 backdrop-blur-md rounded-lg shadow-lg p-6">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-xl font-semibold text-white">{workout.title}</h3>
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                workout.category === 'strength' ? 'bg-blue-500 text-white' : 'bg-green-500 text-white'
              }`}>
                {workout.category}
              </span>
            </div>
      ...
      {workouts.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-400 text-lg">No workouts found. Create your first workout to get started!</p>
        </div>
      )}
    </div>
  );
